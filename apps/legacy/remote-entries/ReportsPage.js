/**
 * Web Component wrapper for Reports page
 * Exposes Vue 2 component as <legacy-reports> custom element
 */
import Vue from 'vue'
  ...
  methods: {
    syncSharedState() {
      if (typeof window !== 'undefined' && window.__MF_SHARED_STATE__) {
        ...
        console.log('[Legacy Reports] Syncing shared state:', shared)
      }
    },
    handleSharedStateChange(state) {
      console.log('[Legacy Reports] Shared state changed:', state)
    }
  }
}

// Create the custom element
const ReportsElement = wrap(Vue, WrappedReports)

// Extend the element to inject styles into Shadow DOM
class StyledReportsElement extends ReportsElement {
  connectedCallback() {
...
if (typeof window !== 'undefined' && !customElements.get('legacy-reports')) {
  customElements.define('legacy-reports', StyledReportsElement)
}

export default StyledReportsElement
